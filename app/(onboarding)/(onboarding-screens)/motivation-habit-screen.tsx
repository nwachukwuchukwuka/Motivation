...
const HABIT_OBSTACLES = [
  { label: "No immediate effect", icon: "eye-off" },
  { label: "Lose momentum", icon: "clock" },
  { label: "Nothing, I'm consistent", icon: "check-circle" },
  { label: "Overwhelmed", icon: "alert-circle" },
  { label: "Haven't found fit", icon: "help-circle" },
  { label: "Where to start?", icon: "map" },
];

const MotivationHabitScreen = () => {
  ...
  const handleContinue = () => {
    router.push("/life-vision-screen");
  };
      ...
      >
        <View className="mb-12">
          <Text className="text-[#E2E8F0] text-4xl font-bold tracking-tighter leading-tight">
            What gets in the way of making motivation a habit?
          </Text>
          <Text className="text-[#94A3B8] text-lg mt-4 leading-relaxed">
            Identify the barriers that hold you back from consistency.
          </Text>
          <View className="w-12 h-1 bg-emerald-500 mt-6" />
        </View>

        <View className="flex-row flex-wrap justify-between">
          {HABIT_OBSTACLES.map((item) => {
...
export default MotivationHabitScreen;
